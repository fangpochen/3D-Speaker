...
RUN pip install --retries 10 --timeout 180 -r requirements_cpu.txt mysql-connector-python && \
    pip install --retries 10 --timeout 180 torch==2.4.1 --index-url https://download.pytorch.org/whl/cpu && \
    pip install --retries 10 --timeout 180 torchaudio==2.4.1 --index-url https://download.pytorch.org/whl/cpu
...
# 创建模型和数据目录
RUN mkdir -p /app/modelscope_hub_cache /app/voice_auth_db/embeddings
